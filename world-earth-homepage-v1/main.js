...
import { PHOTO_GEOGRAPHIC_CONTRACT_V2A } from './life-photo/photo-data-contract.js'
import { createAuroraCinematicCurtainV7 } from './world/aurora.js'
import { createAtmospherePhysicalV2 } from './world/atmosphere.js'
import { createCloudObservationV2 } from './world/cloud.js'
import { createHomepageRealEarth } from './world/earth.js'
import { createHomepageCivilizationBoundaries } from './world/civilization-boundaries.js'
import { createHomepageGlobalCityLights } from './world/civilization-lights.js'
import { HOMEPAGE_WORLD_APPROVED_V2 } from './world/world-contract.js'

const query=new URLSearchParams(location.search)
const datasetMode=query.get('photoDataset')==='founder'?'FOUNDER_VALIDATION':'FIXTURE'
const photoDataset=await createPhotoDatasetProvider({mode:datasetMode,datasetUrl:query.get('photoDatasetUrl')})
...
function render(now){
  ...
  if((userPaused||reducedMotion)&&now-lastRendered<1000/15){requestAnimationFrame(render);return}
  lastRendered=now
  perfFrames++
  ...
  atmosphere.setCam(camera.position); placeThumbnails(); renderer.render(scene,camera); globalCityLights.setView(earth.viewRotY+spin,WORLD_Y); globalCityLights.setCameraDistance(camera.position.distanceTo(world.position)); globalCityLights.setCloudOffset(now*.001*clouds.driftRate/(Math.PI*2)); globalCityLights.render(camera); requestAnimationFrame(render)
}
...
console.info('[homepage-complete-expression-v1] runtime ready; not CANONICAL')
